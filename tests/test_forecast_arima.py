...
class TestARIMAForecasting:
    ...
    def test_forecast_arima_different_horizons(self, sample_production_data, forecast_horizons):
...
class TestARIMAPerformance:
    ...
    def test_arima_different_frequencies(self):
        ...
        daily_series = pd.Series(np.random.randn(100).cumsum() + 1000, index=daily_dates)
        ...
        weekly_series = pd.Series(np.random.randn(52).cumsum() + 1000, index=weekly_dates)
