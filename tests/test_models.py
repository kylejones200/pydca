...
from decline_analysis.models import (
    q_exp, q_hyp, fit_arps, predict_arps, ArpsParams, estimate_reserves
)
...
class TestDeclineCurves:
    ...
    def test_exponential_decline(self):
        ...
        assert all(result[i] >= result[i+1] for i in range(len(result)-1))  # Should decline
...
class TestEdgeCases:
    ...
    def test_zero_production(self):
        ...
        with pytest.raises(ValueError, match="All production values are zero or negative"):
            fit_arps(t, q, kind="exponential")
