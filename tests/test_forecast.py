...
from decline_analysis.forecast_timesfm import forecast_timesfm, check_timesfm_availability
from decline_analysis.forecast_chronos import forecast_chronos, check_chronos_availability
...
class TestTimesFMIntegration:
    ...
    def test_timesfm_forecast_different_horizons(self, sample_production_data, forecast_horizons):
...
class TestChronosIntegration:
    ...
    def test_chronos_forecast_different_horizons(self, sample_production_data, forecast_horizons):
...
class TestForecastConsistency:
    ...
    def test_forecast_index_consistency(self, sample_production_data):
        ...
        assert arps_forecast.index[0] == timesfm_forecast.index[0] == chronos_forecast.index[0]
        
        # All should have the same frequency
        assert arps_forecast.index.freq == timesfm_forecast.index.freq == chronos_forecast.index.freq
    ...
    def test_forecast_value_reasonableness(self, sample_production_data):
        ...
        assert historical_part.iloc[-1] >= forecast_part.iloc[-1] * 0.5  # Allow some flexibility
