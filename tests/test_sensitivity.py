...
class TestSensitivityAnalysis:
    ...
    def test_sensitivity_multiple_scenarios(self):
        ...
        expected_combinations = len(qi_values) * len(di_values) * len(b_values) * len(prices)
        assert len(results) <= expected_combinations  # Some may be filtered by econ limit
...
class TestSensitivityIntegration:
    ...
    def test_sensitivity_with_arps_params_consistency(self):
        """Test that sensitivity results are consistent with direct Arps calculations."""
...
class TestSensitivityErrorHandling:
    ...
    def test_extreme_economic_parameters(self):
        ...
        # Very high opex
        results_high_opex = run_sensitivity(
            param_grid, prices, opex=100.0
        )
        ...
        if len(results_high_discount) > 0:
            ...
            if len(normal_results) > 0:
                assert results_high_discount.iloc[0]['NPV'] < normal_results.iloc[0]['NPV']
