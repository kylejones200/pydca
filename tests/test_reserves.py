...
class TestReservesEstimation:
    ...
    def test_reserves_time_step_impact(self):
        ...
        assert fine_dt['eur'] >= coarse_dt['eur'] * 0.90  # Allow 10% tolerance for numerical integration differences
...
class TestReservesIntegration:
    ...
    def test_reserves_consistency_with_models(self):
        ...
        reserves_result = forecast_and_reserves(params, t_max=120, dt=1.0, econ_limit=10.0)
    ...
    def test_reserves_realistic_scenarios(self):
        ...
        gas_reserves = forecast_and_reserves(
            gas_well,
            t_max=180,  # 15 years
            econ_limit=100.0  # 100 mcf/month
        )
        ...
        assert 6 < len(gas_reserves['t_valid']) <= 181  # 6 months to 15+ years (allow for edge case)
...
class TestReservesErrorHandling:
    ...
    def test_extreme_parameters(self):
        ...
        result = forecast_and_reserves(low_decline, t_max=60)  # Limit time to avoid huge EUR
