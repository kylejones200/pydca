...
class TestDataLoaderIntegration:
    ...
    def test_data_loader_realistic_workflow(self):
        ...
        mock_production_data = pd.DataFrame({
            ...
            'Oil_Production': [1000, 950, 900, 850, 800, 750, 700, 650, 600, 550, 500, 450],
            'Gas_Production': [5000, 4750, 4500, 4250, 4000, 3750, 3500, 3250, 3000, 2750, 2500, 2250]
        })
        
        # Test data processing workflow
        well_data = mock_production_data[mock_production_data['Well_Name'] == 'Well_001']
        
        # Create time series for decline analysis
        oil_series = pd.Series(
            well_data['Oil_Production'].values,
...
class TestDataLoaderErrorHandling:
    ...
    def test_memory_limitations(self):
        ...
        many_months = [f"2020-{i:02d}" for i in range(1, 13)] + \
...
class TestDataLoaderUtilities:
    ...
    def test_data_aggregation(self):
        ...
        well_data = pd.DataFrame({
            ...
            'Oil_Production': [1000, 950, 900, 850, 800, 750, 
                              800, 760, 720, 680, 640, 600]
        })
        
        # Aggregate by well
        well_summary = well_data.groupby('Well_Name').agg({
            'Oil_Production': ['sum', 'mean', 'count'],
            'Date': ['min', 'max']
        })
...
class TestDataLoaderPerformance:
    ...
    def test_large_dataset_handling(self):
        ...
        monthly_summary = large_data.groupby(
            large_data['Date'].dt.to_period('M')
        )['Production'].sum()
