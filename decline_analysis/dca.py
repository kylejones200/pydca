...
def evaluate(y_true: pd.Series,
             y_pred: pd.Series) -> dict:
...
def sensitivity_analysis(param_grid: List[Tuple[float, float, float]],
                        prices: List[float],
                        opex: float,
    ...
    """
    return run_sensitivity(param_grid, prices, opex, discount_rate, t_max, econ_limit, dt)


def economics(production: pd.Series,
              price: float,
              opex: float,
...
def reserves(params: ArpsParams,
            t_max: float = 240,
            dt: float = 1.0,
...
def load_ndic_data(months_list: List[str], 
                   output_dir: str = "ndic_raw") -> pd.DataFrame:
    """
