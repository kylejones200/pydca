...
def plot_forecast(y_true: pd.Series, 
                 y_pred: pd.Series,
    ...
    ax.plot(y_true.index, y_true.values, 'o-', 
...
def plot_decline_curve(t: np.ndarray, q: np.ndarray, 
                      params: dict, 
...
def plot_benchmark_results(results_df: pd.DataFrame, 
    ...
    if metric in results_df.columns:
        ...
        bars = ax.bar(range(len(results_sorted)), results_sorted[metric], 
        ...
        ax.axhline(median_val, color='orange', linestyle='--', alpha=0.7, label=f'Median: {median_val:.1f}')
