...
def detect_production_anomalies(
    ...
    """
    # Use rolling median and MAD (more robust to outliers than mean/std)
    rolling_median = series.rolling(window=3, center=True).median()

    # Calculate MAD (Median Absolute Deviation)
    mad = series.rolling(window=3, center=True).apply(
