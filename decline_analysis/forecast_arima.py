...
def forecast_arima(series: pd.Series,
    ...
    if len(y) < 3:
        ...
        try:
            future_idx = pd.date_range(y.index[-1] + pd.tseries.frequencies.to_offset(freq),
                                       periods=horizon,
                                       freq=freq)
        except Exception:
    ...
    try:
        ...
        if len(y) < 3:
            ...
            try:
                ...
                last_date = series.index[-1]
                future_idx = pd.date_range(last_date, periods=horizon + 1, freq='MS')[1:]
        ...
        if np.isnan(forecast).any():
            ...
            if len(y) >= 2:
                trend = (y.iloc[-1] - y.iloc[0]) / (len(y) - 1)
                forecast = np.array([y.iloc[-1] + trend * i for i in range(1, horizon + 1)])
            else:
        ...
        try:
            ...
            if len(series) >= 2:
                time_diffs = series.index[1:] - series.index[:-1]
                most_common_diff = time_diffs.mode()[0] if len(time_diffs.mode()) > 0 else time_diffs[0]
                future_idx = [last_date + most_common_diff * (i + 1) for i in range(horizon)]
