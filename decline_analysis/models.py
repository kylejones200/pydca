...
def fit_arps(t: np.ndarray,
             q: np.ndarray,
             kind: Literal["exponential", "harmonic", "hyperbolic"] = "hyperbolic") -> ArpsParams:
    ...
    try:
        if kind == "exponential":
            popt, _ = curve_fit(q_exp, t_valid, q_valid, bounds=(0, np.inf), maxfev=10000)
            qi, di = popt
            return ArpsParams(qi=qi, di=di, b=0.0)
        ...
        if kind == "harmonic":
            def q_harm(t, qi, di):
                return qi / (1 + di * t)
            popt, _ = curve_fit(q_harm, t_valid, q_valid, bounds=(0, np.inf), maxfev=10000)
            qi, di = popt
            return ArpsParams(qi=qi, di=di, b=1.0)

        if kind == "hyperbolic":
            popt, _ = curve_fit(q_hyp, t_valid, q_valid, bounds=(0, [np.inf, np.inf, 2.0]), maxfev=100000)
            qi, di, b = popt
