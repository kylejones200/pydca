...
def forecast_chronos(series: pd.Series, horizon: int = 12) -> pd.Series:
    ...
    try:
        ...
        full_index = pd.date_range(start=series.index[0], 
        ...
        warnings.warn("Required libraries not available for Chronos. Using fallback method.")
        return _fallback_chronos_forecast(series, horizon)
...
def _fallback_chronos_forecast(series: pd.Series, horizon: int) -> pd.Series:
    ...
    try:
        ...
        if len(values) >= 2:
            ...
            forecast_values = np.array([last_val * (0.95 ** i) for i in range(1, horizon + 1)])
...
def forecast_chronos_probabilistic(series: pd.Series, 
                                 horizon: int = 12,
                                 quantiles: list = [0.1, 0.5, 0.9]) -> pd.DataFrame:
    """
